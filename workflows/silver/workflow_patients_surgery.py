...
@task(container_image=image_spec_silver_pat)
def process_patient_surgery(arrow_table: pa.Table) -> pa.Table:
    """Aplica as regras de negócio e extrai apenas o domínio Cirúrgico."""
    df = arrow_table.to_pandas()
    
    orchestrator = PatientFilterOrchestrator()
    # Ignoramos a demographics_df e quarantine_df usando '_'
    _, surgery_df, _ = orchestrator.apply_filters(df)

    return pa.Table.from_pandas(surgery_df)

@workflow
def workflow_patients_surgery() -> bool:
    # 1. Extração
    raw_data = read_bronze_patients()
    
    # 2. Transformação (Apenas Cirurgia)
    surgery_data = process_patient_surgery(arrow_table=raw_data)

    # 2.5 Quality Gate
    validated_data = quality_gate(
        data_pa=surgery_data, 
        contract_path=SETTINGS.contract_patients_surgery_silver
    )
    ...
    write_status = write_dataframe_to_iceberg(
        ...
        table_name=SETTINGS.table_patients_silver_surgery, 
